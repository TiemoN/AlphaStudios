...
public class Tank2 : MonoBehaviour
{
    ...
    void Update()
    {
        if (advancedControl)
        {
            ...
            Tanktop.transform.Rotate(Vector3.up * rotspeed * Input.GetAxis("P2HorizontalRight") * Time.deltaTime);
        }
        else
        {
            if (Input.GetAxis("P2Vertical") != 0 || Input.GetAxis("P2Horizontal") != 0)
            {
                winkel = Mathf.Atan2(Input.GetAxis("P2Horizontal"), Input.GetAxis("P2Vertical")) * Mathf.Rad2Deg;
                if (winkel < 0)
                {
                    winkel = winkel + 360;
                }
            ...
            if (Input.GetAxis("P2VerticalRight") != 0 || Input.GetAxis("P2HorizontalRight") != 0)
            {
                winkel2 = Mathf.Atan2(Input.GetAxis("P2VerticalRight"), Input.GetAxis("P2HorizontalRight")) * Mathf.Rad2Deg;
        ...
        if (Input.GetAxis("P2LeftTrigger") != 0)
        {
            switch (powerup)
            {
                //speed
